 ...
 */
class ExtcontactViewContacts extends JViewLegacy
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		ExtcontactHelper::addSubmenu('contacts');
	...
	protected function addToolbar()
	{
		...
		JToolbarHelper::title(JText::_('COM_EXTCONTACT_MANAGER_CONTACTS'), 'address contact');

		if ($canDo->get('core.create') || (count($user->getAuthorisedCategories('com_extcontact', 'core.create'))) > 0)
		{
		...
		// Add a batch button
		if ($user->authorise('core.create', 'com_extcontact') && $user->authorise('core.edit', 'com_extcontact') && $user->authorise('core.edit.state', 'com_extcontact'))
		{
		...
		if ($user->authorise('core.admin', 'com_extcontact'))
		{
			JToolbarHelper::preferences('com_extcontact');
		}
...
//		JToolbarHelper::help('JHELP_COMPONENTS_CONTACTS_CONTACTS');
		...
		'filter_category_id',
		JHtml::_('select.options', JHtml::_('category.options', 'com_extcontact'), 'value', 'text', $this->state->get('filter.category_id'))
		);
	...
	protected function getSortFields()
	{
		return array(
				...
				'ul.name' => JText::_('COM_EXTCONTACT_FIELD_LINKED_USER_LABEL'),
