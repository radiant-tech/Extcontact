...
/**
 * Extcontact Model to view/manage a single contact
 */
class ExtcontactModelContact extends JModelForm
{
	 ...
	 * @var		string
	 */
	protected $_context = 'com_extcontact.contact';
	...
	protected function populateState()
	{
		...
		if ((!$user->authorise('core.edit.state', 'com_extcontact')) &&  (!$user->authorise('core.edit', 'com_extcontact'))){
	...
	{
		// Get the form.
		$form = $this->loadForm('com_extcontact.contact', 'contact', array('control' => 'jform', 'load_data' => true));
		if (empty($form))
		{
			return false;
		}
	...
	{
		$data = (array) JFactory::getApplication()->getUserState('com_extcontact.contact.data', array());
	...
	public function &getItem($pk = null)
	{
		...
		if (!isset($this->_item[$pk]))
		{
			try
			{
				...
				if (empty($data))
				{
					JError::raiseError(404, JText::_('COM_EXTCONTACT_ERROR_CONTACT_NOT_FOUND'));
				}
				...
				$data->tags->getItemTags('com_extcontact.contact', $data->id);
	...
	protected function getContactQuery($pk = null)
	{
		...
		if ($pk)
		{
			...
			try
			{
				...
				if (empty($result))
				{
					throw new Exception(JText::_('COM_EXTCONTACT_ERROR_CONTACT_NOT_FOUND'), 404);
				}
