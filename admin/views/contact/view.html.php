...
class ExtcontactViewContact extends JViewLegacy
{
	...
	protected function addToolbar()
	{
		...
		$canDo		= JHelperContent::getActions('com_extcontact', 'category', (int) $this->item->catid);

		JToolbarHelper::title(JText::_('COM_EXTCONTACT_MANAGER_CONTACT'), 'address contact');
		...
		if ($isNew)
		{
			...
			if ($isNew && (count($user->getAuthorisedCategories('com_extcontact', 'core.create')) > 0))
			{
		...
		else
		{
			...
			{
				JToolbarHelper::versions('com_extcontact.contact', $this->item->id);
			}
